...
<title>Maintenance Billing - Meridian Heights CHS</title>
  ...
  <div class="main">
    <div class="content-wrap">
      ...
      <div class="topbar">
        ...
        <div class="meta">Current Billing Cycle: <b><?= date('M Y') ?></b><br>Fixed ₹10,000 / flat</div>
      </div>
      ...
      <div class="stats">
        <div class="stat"><div class="label">Total Billed</div><div class="val">₹ 8.40L</div><div class="sub">84 flats</div></div>
        <div class="stat"><div class="label">Collected</div><div class="val">₹ 6.55L</div><div class="sub">78% collection rate</div></div>
        <div class="stat warn"><div class="label">Overdue</div><div class="val">₹ 1.84L</div><div class="sub">23 flats pending</div></div>
        <div class="stat"><div class="label">Late Fee Rule</div><div class="val">₹200</div><div class="sub">+1.5% after due date</div></div>
      </div>

      <div class="controls">
        <div class="chips">
          <div class="chip active">All bills</div>
          <div class="chip">Paid</div>
          <div class="chip">Overdue</div>
          <div class="chip">Pending</div>
        </div>
        <div style="display:flex; gap:10px;">
          <button class="btn" onclick="document.getElementById('genbill').classList.add('open')">＋ Generate Bill Cycle</button>
        </div>
      </div>

      <div class="ledger">
        <div class="lrow head" style="grid-template-columns:90px 1.2fr 110px 110px 110px 100px;">
          <div>Flat</div><div>Billing Cycle</div><div style="text-align:right">Amount</div><div style="text-align:center">Due Date</div><div style="text-align:center">Status</div><div style="text-align:center">Action</div>
        </div>
        
        <?php if (!empty($bills)): ?>
          <?php foreach ($bills as $b): ?>
            <div class="lrow" style="grid-template-columns:90px 1.2fr 110px 110px 110px 100px;">
              <div class="flat"><?= htmlspecialchars($b['flat_number']) ?></div>
              <div class="owner">Cycle: <?= htmlspecialchars($b['billing_cycle']) ?><br><small><?= htmlspecialchars($b['charge_basis']) ?></small></div>
              <div class="amt">₹ <?= number_format($b['amount'], 2) ?></div>
              <div style="text-align:center; font-family:'IBM Plex Mono',monospace; font-size:12.5px;"><?= htmlspecialchars($b['due_date']) ?></div>
              <div style="display:flex; justify-content:center">
                <span class="status <?= strtolower($b['status']) ?>"><?= htmlspecialchars($b['status']) ?></span>
              </div>
              <div style="text-align:center">
                <button class="btn" style="padding:4px 10px; font-size:11px;" onclick="document.getElementById('collect').classList.add('open')">Collect</button>
              </div>
            </div>
          <?php endforeach; ?>
        <?php else: ?>
          <!-- Demo bills if DB is fresh -->
          <div class="lrow" style="grid-template-columns:90px 1.2fr 110px 110px 110px 100px;">
            <div class="flat">A-102</div><div class="owner">Cycle: 2026-08<br><small>Fixed Standard Charge</small></div>
            <div class="amt">₹ 10,000.00</div><div style="text-align:center; font-family:'IBM Plex Mono',monospace; font-size:12.5px;">2026-09-10</div>
            <div style="display:flex; justify-content:center"><span class="status paid">Paid</span></div>
            <div style="text-align:center"><button class="btn" style="padding:4px 10px; font-size:11px;" onclick="document.getElementById('collect').classList.add('open')">Collect</button></div>
          </div>
          <div class="lrow" style="grid-template-columns:90px 1.2fr 110px 110px 110px 100px;">
            <div class="flat">B-304</div><div class="owner">Cycle: 2026-08<br><small>Fixed Standard Charge</small></div>
            <div class="amt">₹ 11,500.00</div><div style="text-align:center; font-family:'IBM Plex Mono',monospace; font-size:12.5px;">2026-08-10</div>
            <div style="display:flex; justify-content:center"><span class="status overdue">Overdue</span></div>
            <div style="text-align:center"><button class="btn" style="padding:4px 10px; font-size:11px;" onclick="document.getElementById('collect').classList.add('open')">Collect</button></div>
          </div>
        <?php endif; ?>
      </div>
